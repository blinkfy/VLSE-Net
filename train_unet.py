...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train the U-Net baseline for SEM pore segmentation."
    )
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Use the same source-level manifests as VLSE-Net for paper reproduction."
        ),
    )
    parser.add_argument(
        "--val-ratio",
    ...
    parser.add_argument(
        "--report-dir",
        type=Path,
        default=Path("./reports/latest_run_unet"),
    )
...
def evaluate(
    ...
    for images, masks, _, _ in tqdm(loader, desc="eval", leave=False):
        images = images.to(device, non_blocking=True)
        masks = masks.to(device, non_blocking=True)
...
def main() -> None:
    ...
    if split_source != "manifest":
        print(
            ...
            "For reproducing the paper baseline, use the same source-level "
            "manifests as VLSE-Net."
        )
    ...
    records: list[dict] = []
    best_checkpoint = report_dir / "best_unet.pt"
    last_checkpoint = report_dir / "last_unet.pt"
